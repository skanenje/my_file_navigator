...
from tools.crewai_tools import ReadFileTool, WriteFileTool, RunShellTool, SearchProjectTool, ExplainCodeTool, GitStatusTool, GitSafeCommandTool, GenerateTestsTool, AnalyzeErrorTool, RunDiagnosticsTool
...
def create_file_navigator_agent():
    ...
    agent = Agent(
        ...
        llm=llm,
        tools=[ReadFileTool(), WriteFileTool(), RunShellTool(), SearchProjectTool(), ExplainCodeTool(), GitStatusTool(), GitSafeCommandTool(), GenerateTestsTool(), AnalyzeErrorTool(), RunDiagnosticsTool()],
        verbose=True
    )
    return agent
